...
<head>
    ...
    <style>
        ...
        html, body {
            ...
            margin: 0;
            padding: 0;
            overflow-x: hidden; /* Mencegah scroll horizontal pada html/body */
        }
        ...
        <div class="main-content"> {{-- Cukup gunakan class "main-content" dan atur di CSS --}}
            ...
            <main class="content p-4 flex-grow-1"> {{-- Menambahkan flex-grow-1 agar konten mengisi ruang --}}
                <div class="d-flex justify-content-between align-items-center mb-4"> {{-- Tambahkan align-items-center --}}
                    <h2 class="mb-0 text-dark">Daftar Kandidat</h2> {{-- Tambahkan text-dark --}}
                    <a href="{{ route('candidate.create') }}" class="btn btn-primary btn-color">
                            ...
                            <table class="table table-hover align-middle"> {{-- Menambahkan align-middle --}}
                                <thead class="table-dark">
                                    <tr>
                                        ...
                                        <th scope="col" class="text-center">Aksi</th> {{-- text-center --}}
                                    </tr>
                                </thead>
                                <tbody>
                                    @forelse ($candidates as $candidate)
                                        <tr>
                                            <td width="100">
                                                <img src="{{ asset('photos/' . $candidate->photo) }}"
                                                    class="img-thumbnail rounded"
                                                    alt="{{ $candidate->nama }}" {{-- Tambahkan alt --}}
                                                    style="max-height: 80px; width: 80px; object-fit: cover;"> {{-- Tambahkan width --}}
                                            </td>
                                            <td>{{ $candidate->nama }}</td>
                                            <td>{{ $candidate->npm }}</td>
                                            <td>{{ $candidate->fakultas }}</td>
                                            <td>{{ $candidate->prodi }}</td>
                                            <td class="text-center text-nowrap"> {{-- text-center dan text-nowrap --}}
                                                <a href="{{ route('candidate.edit', $candidate->id) }}"
                                                    class="btn btn-warning btn-sm me-2" title="Edit Kandidat"> {{-- me-2 dan title --}}
                                                    <i class="bi bi-pencil-square"></i> Edit
                                                </a>
                                                <form action="{{ route('candidate.destroy', $candidate->id) }}"
                                                    method="POST" class="d-inline"
                                                    onsubmit="return confirm('Yakin ingin menghapus kandidat {{ $candidate->nama }}? Aksi ini tidak dapat dibatalkan.')"> {{-- Pesan konfirmasi lebih jelas --}}
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="btn btn-danger btn-sm" title="Hapus Kandidat">
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="6" class="text-center py-4 text-muted">
                                                <i class="bi bi-info-circle me-1"></i> Belum ada kandidat terdaftar.
                                            </td>
                                        </tr>
                                    @endforelse
                                </tbody>
